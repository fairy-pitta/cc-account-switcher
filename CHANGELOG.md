...
- `ccs rate-setup` and `ccs statusline-setup` now write into `~/.claude/settings.json`, the user-level settings file Claude Code actually reads. They previously wrote to `~/.claude/settings.local.json`; a `settings.local.json` is project-scoped (`<project>/.claude/settings.local.json`), so the hook and the statusline installed there never ran. Both commands (and `--disable`) clear the inert entries an earlier version left in that file
- `statusline-setup` no longer overwrites a statusline you configured yourself: it stops and prints the existing command, and replaces it only with `--force`. `--disable` still leaves a statusline that isn't ours alone
