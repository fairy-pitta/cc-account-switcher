...
- **`--timeout` escalates TERM→KILL.** The watchdog sends SIGTERM, waits a 2s grace, then SIGKILL, so a child that traps/ignores SIGTERM is still bounded by the deadline.
- **Deferred (follow-up):** live stderr streaming; killing the watchdog's `sleep` on a fast success; a single EXIT trap to collapse the repeated cleanup calls; TERM→KILL escalation on the signal/cleanup path (only the `--timeout` path escalates today).
