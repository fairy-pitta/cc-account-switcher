...
**検出** はフォーマット非依存です。失敗したコマンドの stderr と stdout の末尾行を、文書化されたレート制限マーカー（`(429)` エラー・`rate-limit`/`rate_limit`・`overloaded`・`usage limit`）で grep します。これらが見つからず、かつコマンドが失敗した場合は Usage API チェック（5時間枠と7日枠の最大値）にフォールバックしてアカウントの枯渇を判定します。
...
| `--limit-threshold N` | 95 | 二次的なレート制限確認とスイッチ後ヘルスチェックで使う使用率の閾値（%） |
| `--timeout SEC` | なし | 1 回の試行の時間制限。子プロセスグループを kill。stdin の読み取りも制限 |
| `--no-proactive` | — | 実行前の使用率チェックをスキップ |
| `--no-stdin` | — | stdin を読まず、子プロセスに `/dev/null` を渡す（下記の stdin 注記を参照） |
...
| 124 | `--timeout` でタイムアウト（または stdin 読み取りのタイムアウト） |
| 3 | 全アカウント枯渇。stderr に機械可読な `ccs-run: exhausted accounts=N attempts=M` を出力 |
| 4 | `--max-attempts` に到達し、健全なアカウントに切り替えたが再試行しなかった。`ccs-run: max-attempts accounts=N attempts=M` を出力。使えるアカウントが残っているのにオーケストレーターが作業を止めないよう、3 とは区別 |
| 2 | 内部切り替えエラー |

stdin はテンポラリファイルにスプールされ、各試行で再生されます（リトライがバイト単位で同一の入力を受け取れるように）。成功した試行の stdout のみ出力されます。

> **IMPORTANT — stdin でブロックし得る:** スプールは先頭で EOF まで読み切るため、stdin を読まないコマンドでも、開いたままの長寿命パイプを継承していると（オーケストレーターが stdin を継承して子プロセスを起動する場合によくある）、最初の試行の前に `ccs run` がブロックします。プロンプトを引数で渡す場合（`claude -p "..."`）は **`--no-stdin`**（または `</dev/null`）を使ってください。`--timeout` も stdin 読み取りを制限します。

> **IMPORTANT — 冪等性:** レート制限で失敗した試行が、制限に達する前にすでに副作用のあるツール呼び出しを実行している可能性があります。デフォルトのリトライはコマンドを最初から再実行するため、副作用が重複することがあります。コマンドを冪等に設計するか、`--max-attempts 1`（次回の呼び出し用に健全なアカウントへ1回だけ切り替えますが、その場での再試行はしません）を使ってください。
